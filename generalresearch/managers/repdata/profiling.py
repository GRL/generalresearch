...
def get_profiling_library(
    ...
    if question_ids:
        ...
        filters.append("lucid_id IN %(question_ids)s")
    ...
    if pks:
        params["pks"] = pks
        filters.append("(lucid_id, country_iso, language_iso) IN %(pks)s")
    filter_str = " AND ".join(filters)
    filter_str = "WHERE " + filter_str if filter_str else ""
    res = sql_helper.execute_sql_query(
        query=f"""
            SELECT *
            FROM `thl-repdata`.`repdata_question` q
            {filter_str}
        """,
        params=params,
    )
    ...
    qs = [RepDataQuestion.from_db(x) for x in res]
    return qs
